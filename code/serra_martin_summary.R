...
df <- df_raw %>%
  # Drop 50 completely blank rows (visita = NA; authors' Python does the same)
  filter(!is.na(visita)) %>%
...
# ---- 2. Richness summary helper (unique species & interactions per group) ----
...
# ---- 3. Richness by habitat patch (6 individual sites) ----
...
# ---- 4. Richness by habitat type (Dune / Pine forest / Rocky coastal) ----
...
# ---- 5. Richness by sampling method (obs vs rpi camera) ----
...
# ---- 6. Pairwise compositional overlap between habitat patches ----
# Jaccard similarity (presence/absence) and Bray-Curtis similarity
# (1 - dissimilarity, abundance-weighted) for plants, pollinators & interactions.
...
# ---- 7. Pairwise compositional overlap between habitat types ----
...
# ---- 8. Species & interactions exclusive to each habitat type ----
